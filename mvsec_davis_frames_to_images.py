...
def run():
    os.mkdir("%s/davis_left"%FLAGS.output_folder)
    f=open("%s/davis_left/timestamps.txt"%FLAGS.output_folder, "a+")

    assert FLAGS.bag is not ''
    assert FLAGS.topic is not ''
    print('Saving mvsec davis frames images from %s...' % FLAGS.bag)
    ...
    for image in images:
        ...
        if FLAGS.output_folder:
            # Save to images
            cv2.imwrite('%s/davis_left/frame_%010d.png' % (FLAGS.output_folder, i), curr_img)
